...
SELECT_DESCRIPTIONS: list[VeSyncSelectEntityDescription] = [
    ...
    # drying_mode for humidifiers (Levoit 6000S)
    VeSyncSelectEntityDescription(
        key="drying_mode",
        translation_key="drying_mode",
        options=["off", "low", "high"],
        icon="mdi:weather-sunny",  # Icon representing drying/heating
        exists_fn=lambda device: is_humidifier(device) and getattr(device, "supports_drying_mode", False),
        select_option_fn=lambda device, value: (
            device.turn_off_drying_mode() if value == "off" else device.turn_on_drying_mode(value)
        ),
        current_option_fn=lambda device: (
            "off" if not getattr(device.state, "drying_mode_enabled", False)
            else getattr(device.state, "drying_mode_level", "low")
        ),
    ),
]
